...
t = np.arange(0, d/sample_rate, 1/sample_rate)
centered = ask*clock
...
# normalise = np.transpose(sync) * np.exp((0.3)*1j)*1/0.4
 
# plt.plot(np.unwrap(np.angle(centered)))

unwrap = np.unwrap(np.angle(sync))
diff_angle = np.diff(unwrap)
sync= np.abs(sync[0:int(len(diff_angle))]) * (np.cos(diff_angle) + 1j*(np.sin(diff_angle))) *  np.exp((0*np.pi)*1j)


normalise = np.transpose(sync) *  np.exp((np.pi*0)*1j)
...
for k in range (0,int(len(normalise))):
        if (normalise[k] > -0.5 and np.real(normalise[k]) < -0.3  ):
            recovery.append("00")
        elif (normalise[k] > -0.3 and normalise[k] < 0):
            recovery.append("01")
        elif (normalise[k] >  0 and normalise[k] < 0.3):
            recovery.append("11")
        elif (normalise[k] > 0.3 and normalise[k] < 0.5):
            recovery.append("10")
...
# 01011110 01001001 00100111 01100100 00100000 01110100 01100101 01101100 
# 01101100 00100000 01111001 01101111 01110101 00100000 01100001 00100000 
# 01100011 01101000 01100101 01101101 01101001 01110011 01110100 01110010 
# 01111001 00100000 01101010 01101111 01101011 01100101 00100000 01100010 
# 01110101 01110100 00100000 01001001 00100000 01101011 01101110 01101111 
# 01110111 00100000 01001001 00100000 01110111 01101111 01110101 01101100 
# 01100100 01101110 00100111 01110100 00100000 01100111 01100101 01110100 
# 00100000 01100001 00100000 01110010 01100101 01100001 01100011 01110100 
# 01101001 01101111 01101110 00101110 00100100
# #######################################################################
