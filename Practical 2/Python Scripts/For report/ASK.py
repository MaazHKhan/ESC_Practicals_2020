...
# plt.plot(diff_angle)
normalise = np.abs(sync[0:int(len(diff_angle))]) * (np.cos(diff_angle) + (1j*(np.sin(diff_angle)))) *np.exp((np.pi*0)*1j)
...
for k in range (0,int(len(normalise))):
        if (normalise[k] < -0.3):
            recovery.append("00")
        elif (normalise[k] > -0.3 and normalise[k] < 0.04):
            recovery.append("01")
        elif (normalise[k] >  00.04 and normalise[k] < 0.3):
            recovery.append("11")
        elif (normalise[k] > 0.3):
            recovery.append("10")
