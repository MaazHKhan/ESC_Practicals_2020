...
clock = 1*np.cos(2*np.pi*freq*clock_range)

msk_sync = []
for i in range (0,d-1):
    if (clock[i] > clock[i-1] and clock[i] > clock[i+1]):
        msk_sync.append(msk[i])  


unwrap = np.unwrap(np.angle(msk_sync))
diff_angle = np.diff(unwrap) 
sync = np.abs(msk_sync[0:int(len(diff_angle))]) * (np.cos(diff_angle) + 1j*(np.sin(diff_angle))) *np.exp((np.pi/2)*1j)
